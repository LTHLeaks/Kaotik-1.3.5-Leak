package com.mawuote.client.gui.click.components.impl;

import com.mawuote.api.manager.module.*;
import com.mawuote.client.gui.click.components.*;
import com.mawuote.api.manager.value.*;
import com.mawuote.api.manager.value.impl.*;
import java.util.*;
import com.mawuote.client.modules.client.*;
import java.awt.*;
import net.minecraft.client.gui.*;
import com.mawuote.*;

public class ModuleComponent extends Component
{
    Module module;
    private final ArrayList<Component> subButtons;
    boolean open;
    
    public ModuleComponent(final Module module, final int x, final int y, final Frame parent) {
        super(x, y, parent);
        this.module = module;
        this.subButtons = new ArrayList<Component>();
        this.open = false;
        int tempY = y + 14;
        if (module.getValues() != null && !module.getValues().isEmpty()) {
            for (final Value setting : module.getValues()) {
                if (setting instanceof ValueBoolean) {
                    this.subButtons.add(new BooleanComponent((ValueBoolean)setting, this, tempY));
                    tempY += 14;
                }
                if (setting instanceof ValueNumber) {
                    this.subButtons.add(new NumberComponent((ValueNumber)setting, this, tempY));
                    tempY += 14;
                }
                if (setting instanceof ValueEnum) {
                    this.subButtons.add(new ModeComponent((ValueEnum)setting, this, tempY));
                    tempY += 14;
                }
                if (setting instanceof ValueString) {
                    this.subButtons.add(new StringComponent((ValueString)setting, this, tempY));
                    tempY += 14;
                }
                if (setting instanceof ValueColor) {
                    this.subButtons.add(new ColorComponent((ValueColor)setting, this, tempY));
                    tempY += 100;
                }
                if (setting instanceof ValuePreview) {
                    this.subButtons.add(new PreviewComponent((ValuePreview)setting, this, tempY));
                    tempY += 70;
                }
                if (setting instanceof ValueBind) {
                    this.subButtons.add(new BindComponent((ValueBind)setting, this, tempY));
                    tempY += 14;
                }
            }
        }
    }
    
    @Override
    public void drawScreen(final int mouseX, final int mouseY) {
        super.drawScreen(mouseX, mouseY);
        Gui.func_73734_a(this.getX(), this.getY(), this.getX() + this.getWidth(), this.getY() + 14, this.module.isToggled() ? ModuleColor.getColor() : new Color(42, 42, 42).getRGB());
        Gui.func_73734_a(this.getX() - 1, this.getY(), this.getX(), this.getY() + 14, new Color(30, 30, 30).getRGB());
        Gui.func_73734_a(this.getX() + this.getWidth(), this.getY(), this.getX() + this.getWidth() + 1, this.getY() + 14, new Color(30, 30, 30).getRGB());
        Kaotik.FONT_MANAGER.drawString(this.module.getTag(), (float)(this.getX() + 3), (float)(this.getY() + 3), Color.WHITE);
        if (this.open && !this.getSubButtons().isEmpty()) {
            for (final Component button : this.getSubButtons()) {
                button.drawScreen(mouseX, mouseY);
            }
        }
    }
    
    @Override
    public void mouseClicked(final int mouseX, final int mouseY, final int mouseButton) {
        super.mouseClicked(mouseX, mouseY, mouseButton);
        if (mouseX >= this.getX() && mouseX <= this.getX() + this.getWidth() && mouseY >= this.getY() && mouseY <= this.getY() + this.getHeight()) {
            if (mouseButton == 0) {
                this.module.toggle();
            }
            else if (mouseButton == 1) {
                this.open = !this.open;
            }
        }
    }
    
    public boolean isOpen() {
        return this.open;
    }
    
    public ArrayList<Component> getSubButtons() {
        return this.subButtons;
    }
}
